...
def cdr_info(request,page,order_type):
    ...
    if request.method=="GET":
        #获取数据库中所有的数据
        all_cdr_data=Unique.objects.filter(history__person__username=username).order_by(order_type)
        ...
        if range_n.is_valid():
            ...
            print(error_msg)
        #获取数据库中所有的数据
        all_cdr_data=Unique.objects.filter(history__person__username=username).order_by(type_n)
...
def download_data(request):
        ...
        if True:
            try:
                ...
                if not os.path.exists(os.path.join(result_path,result_tar)):
                    os.system("tar -czvf %s %s"%(result_tar,folder_n.split(".zip")[0]))
    ...
    #假如method是GET那么就把所有的数据都提取出来，展示在界面上
        ...
        all_history=History.objects.filter(person=MyUser.objects.get(username=username)).order_by("-pubDate")
      
        return render(request,"data_record/download_data.html",{"all_data":all_history})
...
def record_abnormal(path_n,abnormal_file,scfv_files,common_files,unique_files):
    os.chdir(path_n)
    abnormal_file_num=0
    print("cheeeeeeeeeeeeeeeeeeeeeee",abnormal_file,scfv_files,common_files,unique_files)
        ...
        with open(abnormal_file,"r") as content:
            ...
            for line_n in all_lines:
                if line_n.startswith(">"):
                    ...
                    print("nnnnnnnnnnnnnnnnnnn",name_n,"xxxx",scfv_files,common_files,unique_files,common_files.count(name_n),type(name_n))
...
def count_seq(path_n,scfv_char,fab_chars):
    ...
    unique_ids=list(set(vh_files.keys())-set(vl_files.keys()))+list(set(vl_files.keys())-set(vh_files.keys()))
...
def check_num(path_n,scfv_char,fab_chars):
    ...
    scfv_files,common_ids,unique_ids=count_seq(path_n,scfv_char,fab_chars)
    ...
    abnormal_num=record_abnormal(path_abnormal,abnormal_fasta,scfv_files,common_ids,unique_ids)
...
def upload(request):
    if request.method=="POST":
        ...
        if myFile :
            ...
            all_files=os.listdir(os.path.join(target_dir,myFile.name.split(".zip")[0]))
            seq_num=0
            for file_n in all_files:
                if file_n.endswith(".seq"):
                    seq_num+=1
	...
	#假如method不是post
        ...
        serialnum_all=SerialNum.objects.filter(user__username=request.user.username)
