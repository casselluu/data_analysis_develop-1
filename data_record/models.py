...
class Project(models.Model):
    ...
    #相关的信息
    info=models.TextField(max_length=2000,default="暂时无相关信息",verbose_name="相关信息")
    def __str__(self):
...
class SerialNum(models.Model):
    ...
    def get_all_leaders(self):
        return "\n".join( [leader_n.user.username for leader_n in self.leader.all()])
...
class History(models.Model):
    ...
    serialnum=models.ForeignKey(SerialNum,on_delete=models.CASCADE,verbose_name="项目名称")
    #定义分析的时间
    pubDate=models.DateTimeField(verbose_name="数据分析时间")
    #定义分析的人
    person=models.ForeignKey(MyUser,on_delete=models.CASCADE,verbose_name="分析人") 
    def __str__(self):
        return self.folder
    class Meta:
...
class Unique(models.Model):
    ...
    #定义外键（外键对应于历史）
    history=models.ForeignKey(History,on_delete=models.CASCADE,verbose_name="历史")
    def __str__(self):
        return self.seqName
    class Meta:
